  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[61] flex items-center justify-center p-4" onClick={() => setVisible(false)}>
        <div
          className="w-full max-w-2xl rounded-2xl border border-white/[0.08] bg-white/[0.02]/95 shadow-2xl backdrop-blur-xl overflow-hidden animate-in"
        ...
        >
          <div className="flex items-center justify-between border-b border-white/[0.08] px-6 py-4">
            ...
            <button onClick={() => setVisible(false)} className="rounded-lg p-1.5 text-white/40 hover:bg-white/[0.08] hover:text-white/60">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 p-4 sm:grid-cols-2 sm:p-6 max-h-[60vh] overflow-y-auto">
            {shortcuts.map((section) => (
              <div key={section.section}>
                <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-white/40">{section.section}</h3>
                <div className="space-y-1.5">
                  {section.items.map((item) => (
                    <div key={item.description} className="flex items-center justify-between rounded-lg px-2 py-1.5 hover:bg-white/[0.04]">
                      <span className="text-xs text-white/50">{item.description}</span>
                      <div className="flex items-center gap-1">
                        {item.keys.map((key, i) => (
                          <span key={i}>
                            <kbd className="inline-flex h-6 min-w-[24px] items-center justify-center rounded bg-white/[0.08] px-1.5 text-[10px] font-mono text-white/60 border border-white/[0.12]">
                              {key}
                            </kbd>
                            {i < item.keys.length - 1 && <span className="mx-0.5 text-[10px] text-white/15">+</span>}
                          </span>
                        ))}
                      </div>
                    </div>
          ...
          <div className="border-t border-white/[0.08] px-6 py-3 text-center">
            <p className="text-[10px] text-white/25">
              Press <kbd className="rounded bg-white/[0.08] px-1.5 py-0.5 text-[9px] font-mono">?</kbd> to toggle this panel
            </p>
          </div>
        </div>
      </div>
